...
def box_model(t, depth, baseline):
    ...
    # THE FIX: 0.1 days (2.4 hours) is the true width of Kepler-186f's transit
    in_transit = np.abs(t - 0.0) < (0.1 / 2.0)
...
def main():
    ...
    print("running scipy curve fit (for real this time)...")
    ...
    # initial guesses: depth and baseline
    guess = [0.0004, 1.0]
    
    # bounds to prevent negative depth (NaN) errors
    ...
    print(f"duration:      2.40 hrs (fixed)")
    ...
    ax.set_title('Kepler-186f Transit Fit (SciPy Corrected)')
